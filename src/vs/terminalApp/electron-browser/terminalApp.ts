...
(async function () {
	...
	async function load<M, T extends ISandboxConfiguration>(options: ILoadOptions<T>): Promise<ILoadResult<M, T>> {
		...
		// ESM Import - load the terminal window main module
		try {
			let workbenchUrl: string;
			if (!!safeProcess.env['VSCODE_DEV'] && globalThis._VSCODE_USE_RELATIVE_IMPORTS) {
				workbenchUrl = '../terminalApp.desktop.main.js'; // for dev purposes only
			} else {
				workbenchUrl = new URL(`vs/terminalApp/terminalApp.desktop.main.js`, baseUrl).href;
			}
