...
import { TerminalCommandDecorationActions } from '../terminalDecorationActions.js';
...
export class DecorationAddon extends Disposable implements ITerminalAddon, IDecorationAddon {
	...
	private async _getCommandActions(command: ITerminalCommand): Promise<IAction[]> {
		...
		for (const provider of TerminalCommandDecorationActions.providers) {
			const providedActions = await provider.provideActions(this._resource, command);
			if (providedActions?.length) {
				actions.push(...providedActions, new Separator());
			}
		}
